...
# معرف مالك البوت — صلاحية تقنية لإدارة البوت فقط (تصفير العالم)، وليست دوراً داخل اللعبة
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # === قائمة الهجوم مع pagination — تعرض جميع الدول المسجلة ===
    if data.startswith("attack_list_"):
        page_part = data.split("_", 2)[2].split("|", 1)[0]
        page = int(page_part) if page_part.isdigit() else 0
        country = load_country(uid)
        if not country:
            await query.edit_message_text("❌ ليس لديك دولة!", reply_markup=back_war_kb)
            return
        # مصدر أهداف الهجوم هو جميع الدول المسجلة، وليس أعضاء القروب الحالي.
        targets = [c for c in get_all_countries() if c.get("user_id") != uid]
        # ترتيب حسب القوة
    ...
    if data == "how_to_nuke":
        await query.edit_message_text(
            "☢️ *الضربة النووية*\n━━━━━━━━━━━━━━━\n\n"
            "اكتب: `نووي [اسم الدولة]` (الأقوى تلقائياً)\n`نووي تكتيكي [اسم الدولة]`\n`كيميائي [اسم الدولة]`\n\n"
            "📌 *الشروط:*\n"
            "• تحتاج منشأة نووية\n"
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text.startswith("انشاء دولة "):
        ...
        new_country = {
            ...
            "ذهب": 500000, "بنك": 0, "آخر استثمار": 0,
            "وحدات": {}, "مباني": [],
            "انتصارات": 0, "خسائر": 0,
    ...
    if text.startswith("نووي تكتيكي "):
        nuke_command = ("نووي تكتيكي", text[len("نووي تكتيكي "):].strip())
    elif text.startswith("كيميائي "):
        nuke_command = ("كيميائي", text[len("كيميائي "):].strip())
    elif text.startswith("نووي "):
        nuke_command = (None, text[len("نووي "):].strip())

    if nuke_command:
        requested_nuke, target_name = nuke_command
        ...
        # "نووي" يختار الأقوى تلقائياً، بينما الأوامر المحددة تتحقق من السلاح المطلوب.
        if requested_nuke:
            if attacker.get("وحدات", {}).get(requested_nuke, 0) <= 0:
                await reply(f"❌ لا تملك سلاح *{requested_nuke}*!")
                return
            best_nuke = requested_nuke
        else:
...
async def reset_world_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    تصفير العالم: يحذف فقط بيانات اللاعبين والعالم القديم
    (الدول، التحالفات، التصويتات، المناطق، طلبات الانضمام المعلّقة).
    لا يحذف ولا يغيّر أي كود أو تصميم للعبة (الأسلحة/المباني/الوحدات/الثوابت).
    مقيّد بمالك البوت فقط (OWNER_ID) ويتطلب تأكيداً صريحاً لتفادي التنفيذ بالخطأ.
    """
    user = update.effective_user
    if not OWNER_ID or user.id != OWNER_ID:
        return  # تجاهل صامت لغير المخوّلين — لا نكشف وجود الأمر
    if not context.args or context.args[0] != "تأكيد":
        await update.message.reply_text(
            "⚠️ *تحذير: عملية لا يمكن التراجع عنها!*\n\n"
            "سيتم حذف بيانات اللاعبين والعالم الحالي بالكامل:\n"
            "🌍 كل الدول وجيوشها ومواردها\n"
            "🤝 كل التحالفات\n"
            "🗳️ كل التصويتات\n"
            "🗺️ كل السيطرة على المناطق\n"
            "📨 كل طلبات الانضمام المعلّقة\n\n"
            "✅ لن يتأثر تصميم اللعبة إطلاقاً (الأسلحة، المباني، الوحدات، المناطق، كل الثوابت تبقى كما هي).\n\n"
            "للتأكيد النهائي أرسل:\n`/reset_world تأكيد`",
            parse_mode="Markdown"
        )
        return
    countries_col.delete_many({})
    alliances_col.delete_many({})
    votes_col.delete_many({})
    territories_col.delete_many({})
    db["join_requests"].delete_many({})
    await update.message.reply_text(
        "✅ *تم تصفير العالم بنجاح!*\n━━━━━━━━━━━━━━━\n\n"
        "🌑 عالم *عرش الظلال* جديد وفارغ جاهز الآن.\n"
        "كل اللاعبين يبدؤون من الصفر بلا استثناء.\n\n"
        "🛡️ تصميم اللعبة (الأسلحة، المباني، الوحدات، المناطق) لم يتغيّر إطلاقاً.",
        parse_mode="Markdown"
    )
...
if __name__ == "__main__":
    ...
    application.add_handler(CommandHandler("reset_world", reset_world_command))
